import {
  CODE_EXTENSIONS,
  GENERIC_BINARY_EXTENSIONS,
  type IgnoreRuleSet,
  IMAGE_EXTENSIONS,
  type IntrinsicFileType,
  MARKDOWN_EXTENSIONS,
  PDF_EXTENSIONS,
  type PDFYFileSystemEntry,
  type PDFYFileType,
  TEXT_EXTENSIONS,
} from "$lib/types";
...
function classifyFileType(fileName: string, mimeType?: string): IntrinsicFileType {
...
export async function readFileContent(fileHandle: FileSystemFileHandle): Promise<string> {
  try {
    ...
    if (intrinsicType === "code" || intrinsicType === "text" || intrinsicType === "markdown") {
      return await file.text();
    } else if (intrinsicType === "image") {
