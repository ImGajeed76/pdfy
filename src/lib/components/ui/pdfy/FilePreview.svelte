<script lang="ts">
  ...
  let extension: string | undefined = $derived(pdfyFile.name.split(".").pop()?.toLowerCase());
  let isMd: boolean = $derived(extension !== undefined && MARKDOWN_EXTENSIONS.has(extension));
  ...
  let currentRenderModeStore = $derived(
    isMd ? markdownPreviewMode : isHtml ? htmlPreviewMode : isXml ? xmlPreviewMode : null,
  );
  ...
  onMount(async () => {
    ...
    if (pdfyFile.fileType === "graphic") {
      ...
      try {
        ...
        console.warn(`PDFy: Highlighting failed for ${pdfyFile.name} (lang: ${langToUse}):`, e);
      ...
      if (isMd) {
        ...
        if (currentRenderMode === RenderMode.Rendered) {
          ...
          if (hljs.getLanguage("xml")) {
            rawHtmlContent = hljs.highlight(content, { language: "xml" }).value;
        ...
        if (currentRenderMode === RenderMode.Rendered) {
          ...
          if (hljs.getLanguage("xml")) {
            highlightedXmlContent = hljs.highlight(content, { language: "xml" }).value;
            newHighlightLanguageClass = "language-xml";
          } else {
...
<div class="file-preview-item mb-0 p-0 py-1">
  <p class="mb-2 hidden font-mono text-sm font-semibold print:block">
    {pdfyFile.path}
  </p>

  <Collapsible.Root bind:open={isOpen} class="w-full">
    <div
      class="bg-muted/20 sticky top-0 z-10 mb-1 flex items-center justify-between px-3 py-2 backdrop-blur-sm print:hidden"
    >
      <Collapsible.Trigger class="flex cursor-pointer items-center">
        <h3 class="mr-2 flex-grow truncate font-mono text-xs font-semibold" title={pdfyFile.path}>
          {pdfyFile.path}
        </h3>
        <ChevronRight
    ...
    <Collapsible.Content class="border print:border-0">
      <div class="preview-content-area text-sm">
        {#if needsPreCodeWrapper}
          <pre class="hljs m-0 overflow-x-auto rounded-none p-3 text-xs"><code
              class={highlightLanguageClass}>{@html finalHtmlOutput}</code
            ></pre>
        {:else if pdfyFile.fileType === "rendered" && isMd && currentRenderMode === RenderMode.Rendered}
          <div class="prose prose-sm dark:prose-invert markdown-body max-w-none p-3">
...
<style>
  ...
  :global(pre.hljs code .code-line::after) {
    ...
    width: 1px;
    background-color: rgba(128, 128, 128, 0.35); /* A subtle semi-transparent gray */
