...
<div style="padding-left: 0.75rem;">
  {#if entry.kind === "directory"}
    ...
    <Collapsible.Root bind:open={isOpen} class="w-full">
      <div
        class="hover:bg-muted/50 flex w-full items-center {isSelected
          ? 'bg-muted font-semibold'
          : ''} mb-1 rounded-md pl-2"
      >
        <Collapsible.Trigger class="m-0 w-fit p-0">
          <ChevronRight
      ...
      <Collapsible.Content>
        <div class="space-y-1">
          ...
          {:else if isOpen && (!entry.children || entry.children.length === 0)}
            <p class="text-muted-foreground py-1 pl-8 text-xs">(empty)</p>
          {/if}
        </div>
      </Collapsible.Content>
    </Collapsible.Root>
  {:else if entry.kind === "file"}
    ...
    >
      ...
      <!-- Indentation spacer -->
      <Icon class="mr-2 h-4 w-4 {isSelected ? 'text-primary' : 'text-gray-500'}" />
      {entry.name}
    </Button>
  {/if}
</div>
