<script lang="ts">
  ...
  async function handleLoadFolder() {
    ...
    if (result) {
      ...
      toast.error("Failed to load directory. Please ensure you have selected a valid folder.");
    }
  ...
  <style global>
    @media print {
      ...
      .border-r-transparent {
        /* Reset borders that might be on panes */
      ...
      .p-2,
      .p-3 {
        /* Reset paddings if they conflict with page margins */
        padding: 0 !important;
      }
      .h-full {
        height: auto !important;
...
<div class="flex h-screen flex-col print:hidden">
  <header
    class="bg-muted/40 flex h-14 items-center gap-4 border-b px-4 lg:h-[60px] lg:px-6 print:hidden"
  >
