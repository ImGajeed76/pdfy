...
PDFy is a client-side SvelteKit web app that turns source code and text files into searchable, text-based PDFs with syntax highlighting. The user picks a local folder via the File System Access API, browses the file tree, selects files, and exports them via the browser's print dialog. Everything runs in-browser, no backend.
...
- **Print/PDF output:** PDFs are produced by the browser's native print dialog. Print-specific styles live in `src/routes/tool/+page.svelte` under `@media print`.
